...
here::i_am("./03_Output/Precipitation Report.Rmd")
library(here)
...
source("02_Processing/Master_Source.R")
```
### Introduction
Is there a way to align the titles to the middle? Can I also add paragraph indents? 


Station 20, the USGS rain gauge is located at 1333 NW Eastman Parkway. As of the date of the compilation of this document there have been _______ hourly precipitation measurements since its installation on ______-. 


Below is the report for the current `r current_water_year` water year. The report includes:
...
### Results 
The average speed is `r mean(cars$speed)` miles per hour 

```{r,echo=FALSE}
#CALL GRAPH HERE BEFORE YOU DISCUSS THEM. 
monthly_graph
```
